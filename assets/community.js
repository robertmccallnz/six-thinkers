/* Kiwi Dialectic community layer — Kōrero
   ...
   - Graceful fallback if Giscus fails to load (network, blocked, app not installed)
*/
(function () {
  var REPO         = "robertmccallnz/six-thinkers";
  var REPO_ID      = "R_kgDOTLFS_Q";
  var CATEGORY     = "General";
  var CATEGORY_ID  = "DIC_kwDOTLFS_c4DAWjb";
  ...
  function rewriteHead(root) {
    // Replace the Cactus-era lede/meta with kaupapa Māori Giscus wording.
    var lede = root.querySelector('.kc-lede');
    if (lede) {
      lede.innerHTML =
        'Kōrero mai — join the discussion below. Comments are hosted on ' +
        '<a href="https://github.com/robertmccallnz/six-thinkers/discussions" target="_blank" rel="noopener">GitHub Discussions</a> ' +
    ...
    if (meta) {
      meta.innerHTML =
        ...
        'See <a href="/six-thinkers/privacy/">the privacy note</a> for the full kōrero.';
    }
  ...
  function loadGiscus(mount) {
    return new Promise(function (resolve, reject) {
      // Wipe any prior content (e.g. loader) before mounting.
  ...
  function boot() {
    ...
    loadGiscus(mount).catch(function () {
      // Giscus script failed — flag as broken so the fallback message shows.
      markBroken(root);
      // Remove the loader in case it survived.
